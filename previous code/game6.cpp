...
public:
    ...
    void update(){s++;}
    void display(){
        outtextxy(599-40,289-32,"Current Score");
        char aa[10];
        char* a=int_to_string(s,aa,10);
        outtextxy(10,10,a);
        outtextxy(599,289,a);
    }
}life_count;
...
bool game_over(const score &life_count){
...
//std::string s = std::to_string(life_count.get_score());
 //int will be stored here as a string
        ...
        if(u<=640){
            ...
            fall[2].draw();
//            delay(5);
            if(!game_over(t1,t2,t3,u,i)){
                    getch();
                    break;
            }
            //check if bullet strikes the shooter_box
        }
        else{
            u=4;
            t1=rand()%500;
...
//targer:circle
 ...
 //       display(i,j,i+x,j);
    ...
    char final[]="Game over";
    outtextxy(255,255,final);
 //   final = "3000";
    outtextxy(255,300,"3000");
//    life_count.display()
    getch();
