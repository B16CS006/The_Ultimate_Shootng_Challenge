...
class shooter{
    int i,j,x;                                                          //i and j are the corner point of shooter_box and x is the for changing angle of shooter
public:
    ...
    void change(int ii,int jj,int xx){                              //for changing the position of shooter-box and shooter
        i=ii;
        j=jj;
        x=xx;
    }
...
//    cout<<"bullet\n";
    void draw(const circles& target){
        ...
        int l = (target.get_y_coordinate()-k-28)*(50-c)/28 + h+50;
        if(l>=target.get_x_coordinate()-target.get_radius()&& l<=target.get_x_coordinate()+target.get_radius()){
            cleardevice();
            outtextxy(189,611, "Game Over");
            getch();
        }
        delay(30);
    }
